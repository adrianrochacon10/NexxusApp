...
export default function NuevaCategoria() {
  ...
  function agregarAtributo() {
    setAtributos((prev) => [...prev, ""])
  }

  function editarAtributo(i: number, valor: string) {
    setAtributos((prev) => prev.map((a, idx) => idx === i ? valor : a))
  }

  function eliminarAtributo(i: number) {
  ...
  async function handleSubmit(e: { preventDefault: () => void }) {
    ...
    setError("")
    setGuardando(true)
    crearCategoria({ nombre: nombre.trim(), color, atributosBase: atributos })
          ...
          <div style={{ display: "grid", gap: 14 }}>
            ...
            {error && (
              <div role="alert" style={{
                ...
                border: "1px solid rgba(155,74,74,0.4)", background: "rgba(155,74,74,0.1)", color: "var(--error)",
              }}>
                {error}
              </div>
            )}
            ...
            <section className="surface" style={{ display: "grid", gap: 16, padding: 20 }}>
              ...
              <label style={{ display: "grid", gap: 7, fontSize: 13, fontWeight: 500, color: "var(--text-secondary)" }}>
                ...
                <input
                  ...
                  placeholder="Ej. Playeras, Perfumes, Accesorios…"
                  autoFocus
                  required
                />
              </label>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      padding: 2, cursor: "pointer", background: "var(--surface-3)",
                    }}
                  />
                </div>
              </div>
            ...
            <section className="surface" style={{ display: "grid", gap: 16, padding: 20 }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: 13, color: "var(--text-secondary)" }}>
                  Cada atributo se convierte en un campo al crear productos de esta categoría.
                  Ej: Talla, Color, Mililitros, Aroma…
                </p>
              </div>
                ...
                {atributos.map((atributo, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center" }}>
                    <input
                      className="input"
                      value={atributo}
                      onChange={(e) => editarAtributo(i, e.target.value)}
                      placeholder={`Atributo ${i + 1} — Ej. Talla`}
            ...
            <section className="surface elevated" style={{ padding: 20, display: "grid", gap: 14 }}>
              ...
              <div className="surface" style={{ padding: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  ...
                  />
                  <span className="font-display" style={{ fontSize: 22, fontWeight: 500 }}>
                ...
                {atributosValidos.length > 0 ? (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                    {atributosValidos.map((a) => (
                      <span key={a} className="badge" style={{ fontSize: 11 }}>
                        {a}: —
                      </span>
                    ))}
                  </div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: "var(--text-tertiary)" }}>
                Así se verá la categoría en el inventario y en el formulario de productos.
              </p>
            </section>
